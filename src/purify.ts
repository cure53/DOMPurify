...
function createDOMPurify(window: WindowLike = getGlobal()): DOMPurify {
   ...
   */
  const _stripElementEventHandlers = function (element: Element): void {
   ...
   */
  const _stripEventHandlers = function (node: Node): void {
    if (getNodeType && getNodeType(node) === NODE_TYPE.element) {
      _stripElementEventHandlers(node as Element);
    }
  ...
  const _sanitizeElements = function (currentNode: any): boolean {
    ...
    if (
      ...
      if (IN_PLACE) {
        _stripEventHandlers(currentNode);
      }
